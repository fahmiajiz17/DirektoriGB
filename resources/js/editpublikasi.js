document.addEventListener('DOMContentLoaded', (event) => {
    document.querySelectorAll('.edit-btn').forEach(button => {
        button.addEventListener('click', function() {
            ...
            let link = row.querySelector('.link_publikasi').href;
            let tahun = row.querySelector('.thn_terbit').textContent;

            // Populate the modal with the data
            document.getElementById('editJudul').value = jdl_publikasi;
            document.getElementById('editLink').value = link_publikasi;
            document.getElementById('editTahun').value = thn_terbit;
